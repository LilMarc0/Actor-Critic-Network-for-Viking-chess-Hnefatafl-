...
import sys
import time
from ActorCriticCNNLSTM import *
from Board import *
import os
...
def train(sess, env, actor, critic, actor_noise):
    ...
    replay_buffer = ReplayBuffer(300000)        # bufferSize, random_seed
    #atexit.register(ex, replay_buffer, actor, critic)
    if(os.path.isfile('./rBufferCNNLSTM.pickle')):
        print('Loading buffer')
        replay_buffer.load('./rBufferCNNLSTM.pickle')
    ...
    for i in range(500):  # max episodes
        ...
        while not b.gameOver():  # max episode len
            ...
            if iterations % 5000 == 0 and iterations != 0:
                actor.save()
                critic.save()
                replay_buffer.save('./rBufferCNNLSTM.pickle')

            # try:
            if replay_buffer.size() % rmov == 0 and iterations != 0:
                #print('---random move')
                xc = b.randomMove()
                ...
                an = actor_noise()
                a = actor.predict(np.reshape(mem, (1, 5, 9, 9))) + an
                a = list(map(abs, a))
            ...
            #    print(titime() - last_time)
            #print(x)
            #print(replay_buffer.size())
            replay_buffer.add(np.reshape(mem, (5, 9,9)), np.reshape(x, (2)), r, terminal, np.reshape(mem2, (5, 9,9)))
            rot, rot2, fx = b.genFlips(mem, mem2, x)

            # rotatii
            for _ in range(3):
                replay_buffer.add(np.reshape(rot[_], (5, 9,9)),
                                  np.reshape(fx[_], (2)),
                                  r,
                                  terminal,
                                  np.reshape(rot2[_], (5, 9,9)))
...
with tf.Session() as sess:
    aenv = Board()
    aactor = ActorNetwork(sess, [None, 5, 9,9], actiondim, 81, 0.2, 0.05, 50)
    acritic = CriticNetwork(sess, [None, 5, 9,9], actiondim, 0.05, 0.9, 0.85, aactor.get_num_trainable_vars())
    if(os.path.isfile('./actorCNNLSTM/actorCNNLSTM.ckpt.index')):
        aactor.load()
    if (os.path.isfile('./criticCNNLSTM/criticCNNLSTM.ckpt.index')):
        acritic.load()
