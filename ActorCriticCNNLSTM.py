...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        inputs = tflearn.input_data(shape= [None, 5, 9, 9])

        conv = conv_2d(inputs, nb_filter=9, filter_size=3, strides=1)
        conv = tflearn.reshape(conv, [-1, 5, 9*9])

        net = tflearn.lstm(conv, 256, return_seq=True)
        ...
        out = tflearn.fully_connected(
            net, 2, activation='sigmoid', weights_init=w_init)
    ...
    def save(self):
        print('Saving actor...')
        save_path = self.saver.save(self.sess, "./actorCNNLSTM/actorCNNLSTM.ckpt")

    def load(self):
        print('Restoring actor...')
        res_path = self.saver.restore(self.sess, "./actorCNNLSTM/actorCNNLSTM.ckpt")
...
class CriticNetwork(object):
    ...
    def save(self):
        print('saving critic..')
        save_path = self.saver.save(self.sess, "./criticCNNLSTM/criticCNNLSTM.ckpt")

    def load(self):
        print('Restoring critic...')
        self.saver.restore(self.sess, "./criticCNNLSTM/criticCNNLSTM.ckpt")
