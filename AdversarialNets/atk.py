...
class atkActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, action_bound, learning_rate, tau, batch_size):
        ...
        self.inputs, self.out, self.scaled_out = self.create_Aactor_network()
        self.network_params = tf.trainable_variables('ATK')
        print(self.network_params)
        self.target_inputs, self.target_out, self.target_scaled_out = self.create_Aactor_network()
        self.target_network_params = tf.trainable_variables('ATK')[len(self.network_params):]

        with tf.variable_scope('netParams'):
    ...
    def create_Aactor_network(self):
        with tf.variable_scope('ATK'):
            with tf.variable_scope('Ainputs1'):
                inputs = tflearn.input_data(shape=[None, 5, 7, 7])
            with tf.variable_scope('Aconv1'):
                conv = conv_2d(inputs, nb_filter=9, filter_size=3, strides=1, reuse=tf.AUTO_REUSE, scope='DatkConv', trainable=True)
            ...
            with tf.variable_scope('Alstm'):
                net = tflearn.lstm(conv, 256, return_seq=True, reuse=tf.AUTO_REUSE, scope='atk', name='DatkLstm1', trainable=True)
                net = tflearn.dropout(net, 0.6)
                net = tflearn.activations.elu(net)
                net = tflearn.reshape(net, [-1, 5, 256])
            with tf.variable_scope('Alstm1'):
                net = tflearn.lstm(net, 256, reuse=tf.AUTO_REUSE, scope='atk', name='DatkLstm2')
                net = tflearn.dropout(net, 0.6)
                net = tflearn.activations.elu(net)
            ...
            with tf.variable_scope('out'):
                ...
                out = tflearn.fully_connected(
                    net, 588, activation='sigmoid', weights_init=w_init, reuse=tf.AUTO_REUSE, scope='DatkOut')
...
class atkCriticNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, gamma, num_actor_vars):
        ...
        self.inputs, self.action, self.out = self.create_Acritic_network()

        self.network_params = tf.trainable_variables('ATK')[num_actor_vars:]

        self.target_inputs, self.target_action, self.target_out = self.create_Acritic_network()

        self.target_network_params = tf.trainable_variables('ATK')[(len(self.network_params) + num_actor_vars):]
    ...
    def create_Acritic_network(self):
        with tf.variable_scope('ATK'):
            ...
            with tf.variable_scope('conv2'):
                conv = conv_2d(inputs, nb_filter=9, filter_size=3, strides=1, reuse=tf.AUTO_REUSE, scope='atk', name='CatkConv')
                conv = tflearn.reshape(conv, [-1, 5, 63])

            with tf.variable_scope('lstm2'):
                net = tflearn.lstm(conv, 256, return_seq=True, reuse=tf.AUTO_REUSE, scope='atk')
                net = tflearn.dropout(net, 0.6)
                net = tflearn.activations.elu(net)
                net = tflearn.reshape(net, [-1, 1280])

            with tf.variable_scope('fc1'):
                t1 = tflearn.fully_connected(net, 600, reuse=tf.AUTO_REUSE, scope='atk')
            with tf.variable_scope('fc2'):
                t2 = tflearn.fully_connected(action, 600, reuse=tf.AUTO_REUSE, scope='atk')
            ...
            with tf.variable_scope('out2'):
                ...
                w_init = tflearn.initializations.uniform(minval=-0.003, maxval=0.003)
                out = tflearn.fully_connected(net, 1, weights_init=w_init, reuse=tf.AUTO_REUSE, scope='atk')
